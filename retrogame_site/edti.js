...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const resultsList = document.getElementById('resultsList');
    let gamesData = [];
    let currentGameData = null; // game.json data
    let currentEditRow = null;

    // Lataa pelit games.json-tiedostosta
    fetch('games.json')
        .then(response => response.json())
        .then(data => {
            gamesData = data.games;
    ...
    // Lataa valitun pelin tiedot
    loadGameButton.addEventListener('click', function () {
        const selectedGameId = gameSelector.value;
        const selectedGame = gamesData.find(game => String(game.ID) === String(selectedGameId));

        if (selectedGame) {
            // Näytä valittu peli ja hae myös game.json-tiedoston tiedot
            fetchGameJson(selectedGame.ID)
                .then(gameJsonData => {
                    currentGameData = gameJsonData;
                    displayGame(selectedGame, gameJsonData);
                })
                .catch(error => {
                    console.error('Virhe game.json-tiedoston lataamisessa:', error);
                    displayGame(selectedGame, null); // Näytä ainakin games.json-tiedot
                });
        } else {
            console.error('Peliä ei löydy valitulla ID:llä:', selectedGameId);
        }
    ...
    // Hae game.json tiedot
    function fetchGameJson(gameId) {
        return fetch('game.json')
            .then(response => response.json())
            .then(data => data.games.find(game => String(game.ID) === String(gameId)))
            .catch(error => {
                console.error('Virhe game.json-tiedoston lataamisessa:', error);
                throw error;
            });
    ...
    // Näytä pelin tiedot
    function displayGame(game, gameJsonData) {
        document.getElementById('editGameName').innerText = game.game_name.fi;

        // Yhdistä games.json ja game.json tulokset hall_of_fame-listaan
        const combinedResults = game.hall_of_fame.concat(gameJsonData?.hall_of_fame || []);
        resultsList.innerHTML = ''; // Tyhjennä aiemmat tulokset

        combinedResults.forEach(score => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${score.username}</td>
                <td>${score.score}</td>
    ...
    document.getElementById('addScoreForm').addEventListener('submit', function (event) {
        ...
        if (currentGameData) {
            ...
            // Päivitä tulos game.json-tiedostoon
            saveScoreToFile(selectedGameId, { username, score });
            document.getElementById('addScoreForm').reset(); // Tyhjennä lomake
        } else {
            console.error('Pelin tietoja ei ole ladattu.');
        }
    });

    // Tallenna uusi tulos game.json-tiedostoon
    function saveScoreToFile(gameId, scoreData) {
        fetch('game.json')
            .then(response => response.json())
            .then(data => {
            ...
            .catch(error => {
                console.error('Virhe game.json-tiedoston käsittelyssä:', error);
            });
    }
